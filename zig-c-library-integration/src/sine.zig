...
fn write_callback(maybe_outstream: ?[*]c.SoundIoOutStream, frame_count_min: c_int, frame_count_max: c_int) callconv(.C) void {
    _ = frame_count_min;
    ...
    while (frames_left > 0) {
        ...
        while (frame < frame_count) : (frame += 1) {
            ...
            const sample = std.math.sin((seconds_offset + float_frame * seconds_per_frame) * radians_per_second);
            {
                var channel: usize = 0;
